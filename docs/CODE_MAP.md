...
- `docs/sgmanalink-authentication.md`: temporary-name decision, backend
  ...
  alternatives; no provider selected or deployed.
- `docs/block-MElo.md`: exploratory decentralized identity/ranking ledger
...
- `game/sgmanalink/payment.gd` (`SgPayment`): shared referee-side bounded X
...
- `tests/ui/test_sgmanalink_recovery.gd`: live-lobby mana questions,
  cancellation, private searches, triggered payments, divided damage,
  lost-ack/reconnect, journal deduplication, large-combat sockets and
  cache invalidation after refused multi-target payments.
- `tests/ui/test_sgmanalink_hardening.gd`: delayed/missing snapshots, bounded
  outgoing commands, replacement-opponent readiness, redundant publication
...
- `game/card_printings.gd`: catalogue lookup, artwork fallback and per-instance
  cosmetic metadata for local and LAN duel presentation.
- `game/deck_builder/card_variant_dialog.gd`: stone printing chooser with a
  full-card preview; one preferred artwork per name in a saved deck. The
  builder opens it from a floating 32px stone medallion below the preview's
  lower-right corner, over the information area. No existing region moves,
  narrows or reserves space for this overlay.
- `game/art/card_variant_on.png`, `card_variant_off.png`: original round
  picture-frame medallion; generated by `tools/draw_our_art.gd`, inventoried
  with hashes in `game/art/README.md`.
- `tests/ui/test_card_printings.gd`: deck round trips, variant counts, chooser,
  medallion placement/scroll/resize, undo, local instances and LAN
  hidden-information regressions.
- `cards/sets/por/*.gd`: 173 new Portal identities; shared simple, trigger,
  choice, spell and semantic effect-shape families. Six earlier-pack rules
  are reused when Portal is the only optional pack enabled.
- `engine/ai/portal_tactics.gd`: public-information dynamic damage/draw,
  restricted-instant and risky-extra-turn decisions.
- `tools/pack_6_portal.py`, `tools/test_pack_6_portal.py`: pinned-printing
  fetcher, deterministic local builder and bounded archive validation tests.
- `tools/pack_6_duel_audit.gd`: bounded seeded Wizard duels with five Portal
  deck themes in modern and Fifth Edition rules.
- `tools/pack_6_ui_soak.gd`: Portal decks through the live human-clicker
  and demo DuelScreen driver, using the standard soak arguments.
- `tests/cards/test_pack_6_*.gd`, `tests/ui/test_pack_6_integration.gd`:
  catalogue, mechanics, choices, hidden-information invariance and UI tests.
- `docs/pack-6-portal.md`, `docs/pack-6-mechanics-audit.md`: pack contract,
  local installation, rule-family coverage and verification record.
...
│   │                          (SceneTree script) — game/art/'s set
│   │                          glyphs (a Roman II for Unlimited since
│   │                          2026-09-18) and the damage dagger.
│   │                          Portal's circular gate and matching
│   │                          stone faces share _portal() (2026-09-23).
│   │                          Every glyph uses add/sub polygons, ellipses and capsules in
