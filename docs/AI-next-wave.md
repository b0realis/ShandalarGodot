...
|---|---|---|---|
| ~~`develops_late`~~ **DONE 2026-09-10 — BUILT, MEASURED AND REFUSED; every preset ships it OFF** | casting P1 | ~~Sorcerer+~~ no rung | ~~everything cast in main 1; the land drop played before it is needed~~ REPRODUCED and not in doubt: a Wizard on four Forests with an Ironroot Treefolk in hand plays the land, casts the Treefolk and stands at their declare-blockers with all of it shown and nothing open. Built whole — Forge's `castPermanentInMain1` list read off `EffectIntent` and the board (a win, floating mana, haste, a non-creature mana source, what changes THIS combat), the mana sink held with it, and the land drop under Forge's own four guards. **THE ROW'S OWN HAZARD IS REAL AND IT IS THE PILOT'S OWN ATTACK**: a card held for Main 2 is mana that looks open in between, and Big Green's Llanowar Elves — tapped for mana in Main 1 at HEAD, so never an attacker — is sent the moment the hold leaves it untapped (mana sources attacking 1.49 → 3.00 a game, a whole cast fewer each game, first sweep −4.7 ±4.4). `AiPlayer._main2_mana_held` is Forge's `reserveManaSourcesForMain2` and closes it to −0.5. **AND THE ANSWER IS STILL NO**: nine pairs at 1,000 games an arm read −0.5, −0.2, +0.8, −2.0, −0.8, −2.9, −1.1, −3.1, −1.8 — eight of nine against the knob, none clear of its interval, a drift of about a point and a quarter — and **the PAIRED count is the instrument that is clear**: 748 of the 8,825 games that ended differently changed hands, 316 to a win and 432 away, where a fair toss sits at 374 ± 14. The ladder's own rule is that a capability is as good or better one rung up. No half explains it (the land drop alone flips one game each way of 1,000; `pumps_to_attack` pinned off on both seats leaves the loss where it was). What the row needs is an opponent that PUNISHES an open board — `holds_tricks` and a hand read — since hiding a card from a seat that never guesses is a cost with no buyer. Control: the forty-FACTORY pair, `DeckLab/README.md` |
...
| Knob | Designed in | Rung | What it fixes |
|---|---|---|---|
| ~~`reads_race`~~ | combat P1 | Sorcerer+ | ~~the tolerance moves with the two clocks, the block trade margin with it~~ **HALF SHIPPED, HALF REFUSED 2026-09-10.** The BLOCK MARGIN ships and is a wash that removes a malfunction: rung 2 of `_best_block_for` takes a voluntary trade at `attacker_value + 0.5` at twenty life as at four, so our Serra Angel traded itself for a Craw Wurm while one turn from winning at 20 against 4, and our Craw Wurm let an Erhnam Djinn through at 8 life because it was worth ONE POINT more. `AiPlayer._trade_margin` reads P1's three states (−0.5 / +0.5 / +1.5) off two clocks of public numbers, with a dead band of a turn and `RACE_HORIZON` (four turns) under it — the horizon is the tree's, because the clamp saturates on a 20-20 board and the first cut refused a Rasputin Dreamweaver a trade the suite has pinned since 2026-09-04. Twenty-matchup matrix −0.6 to +0.8, 63 won to 48 lost; four pairs at 2 000 an arm +0.6 / +0.2 / +0.1 / +0.0. **THE TOLERANCE HALF — P1's headline — IS REFUSED**: built with the floor and the horizon, it ended 42 games in a win against **170 in a loss** over the eight starter matchups it moves most (White Knights vs Black-Red Raiders −3.1), and without the evasive clock 58 to 165, so it is the clamp itself; it moves the deck P1's own Risk paragraph says it must not. The block half alone reads 16 won to 17 lost on the same eight. **AND IT DOES NOT ANSWER `crack_back_margin`'s question**: `_attack_is_reasonable` returns on `risk < 0` before any bar is read, so the Air Elemental is declared on both arms |
...
| ~~`holds_tricks`~~ | combat P5 / casting P13 | Wizard | ~~the bait attacker sent on the strength of the Giant Growth, its mana booked~~ **SHIPPED 2026-09-10** — a GAIN, and the malfunction is measured rather than argued: over 200 whole games of Big Green vs White Knights the pilot reached declare-blockers holding a pump instant **2 051 times and could no longer pay for it in 531 of them (25.9%)**, because `_held_reserve` books removal, a draw and a counterspell and skips a pump outright. `AiPlayer._trick_reserve` puts the pump's cost into that same reserve, on our own turn only, before the blocks only, and only where there is a BAIT — the rider's own pair of readings asked one phase earlier — so an empty board over there books nothing. Big Green vs White Knights 54.1% → 55.4%, Beast Master (4 Giant Growth) vs White Knights 20.5% → 21.9%, vs Mountain Artillery +0.7, vs Black-Red Raiders +0.4; no single delta clear of its interval and **104 flips won to 28 lost** across the four pairs, which a coin does not do. **P5's OTHER HALF DID NOT REPRODUCE** and is not built: *remember the body's id so the response prefers it* answers a question the response was asked in ONE declare-blockers step in 200 games. Pool fact: the shape is Giant Growth and Righteousness and nothing else, only Big Green holds one of the five starters, so 16 of the 20 starter matchups are byte-identical |
...
|---|---|---|---|
| ~~`counts_the_race`~~ **DONE 2026-09-10 — Wave 4 and the plan closed** | casting P3 | Sorcerer+ | ~~decking as the clock: Millstone, Braingeyser at us, a Timetwister into the smaller library. Two liability rows were closed onto this one and both want the same thing from it — a HORIZON, the turns a game has left~~ REPRODUCED four times, and the first is not a mispricing but a decision the pilot had never made: **a MILLSTONE IS NEVER ACTIVATED** — `{2}, {T}: target player mills two cards` falls out of `AiPlayer._ability_option`'s last `else` ("pumps, regeneration, mana, untaps, unknowns") because nothing there has an arm for a payload that is a card off a LIBRARY, and with their library at **two**, where the mill is the game (CR 704.5b), the option is still `{}`; **our own Millstone made no difference to the pace** — 12 cards against their 30 is a race we hold by two turns and `_library_slack` answered `1 << 20`, "the race is lost already"; **their Millstone priced at 2.60** while the one Disenchant took a Jayemdae Tome at 4.20 with our library at six; and **a Timetwister cast at 11.50 into a library of three** with twenty cards in their graveyard, handing it back at 21. Built as ONE reading: the RATE a library loses cards at (`_mill_rate`, a `{T}` mill counted once because the tap is what makes a rate), the CLOCK in turns (`_deck_clock`), the pace counting turns instead of cards, the mill bought at `LETHAL_WORTH` when it decks them and at a card's worth at the mana sink otherwise, `_mill_relief` for taking one of theirs off the table, and `_hands_back_the_race` for the wheel that shuffles the GRAVEYARDS back (`EffectIntent.wheel_recycles`). **THE HORIZON QUESTION THE FOUR ROWS SENT HERE IS ANSWERED, AND THE ANSWER IS NO** (`docs/ai-difficulty.md` §5): a library is the one quantity in this game that never grows back, so a rate taken off it is a fact and `library ÷ rate` is an honest number of turns — while a combat clock changes the moment a creature is cast (which is why `RACE_HORIZON` saturates at four), a toll stops when its permanent leaves, a rebuild is half hidden in a hand, and what a Moat still answers is a question about the cards left in a LIBRARY, a hidden zone this AI may not read. So the general turn horizon is REFUSED with its reasons written down, the decking clock is the only one built, and it needs no new constant — the bound is `PACE_HORIZON`. **A WASH ON EVERY WIN RATE and it says so** — +1.4 ±3.0, +0.8 ±2.5, +0.6 ±2.1, +0.1 ±1.4, not one clear of its interval — but all four arms positive, 1 084 of 8 000 games ending differently and **88 flipped to a win against 28 away**, where a fair toss over 116 sits at 58 ± 5.4; it ships on `runs_loops`' and `holds_x_burn`'s precedent, and because it removes a malfunction the Lab cannot even reach. **THE POOL CANNOT MEASURE THE MILL AT ALL**: Millstone is the only `MillEffect` in the pool, five decks play one in the maindeck and **NOT ONE LOADS** — `ptcs_regnier`, `ptcs_loconto` and `wc1995_redi`, the three P3 names for its own measurement, among them — so the mill half is pinned by `tests/ai/` alone, as The Rack's slope is. The measurable half is the wheel, in the games P3 itself prescribes (`--lives 400,400`, two long control decks). **AND HALF OF P3's CLAUSE (b) IS NOT BUILDABLE**: the draw-that-decks-us was already `counters_by_shape`'s third shape, and a Millstone ACTIVATION cannot be countered by anything — `TargetSpec.Kind` has `SPELL` and no `ABILITY` |
| ~~`minds_the_vise`~~ **DONE 2026-09-10** | casting P4 | Sorcerer+ | ~~the hand under a Vise or a Rack, the board under a Moat~~ REPRODUCED three times over: a Wizard holding seven with a Black Vise across the table went on drawing (the Jayemdae Tome's tick offered at a hand of five, where `_draw_need` already returns exactly **0.00** — so half of clause (a) was built before the row was written), cast a Grizzly Bears at its printed 4.00, and sent its one Disenchant at a **Jayemdae Tome (4.20) instead of the Vise (1.00)** that was squeezing it for three a turn; a Moat with two Craw Wurms of ours behind it priced at **3.20**. Built as three readings, all of them zero with no such permanent on the table: THE ROOM, THE RELIEF and THE PRICE. **THE NOTE'S RACK CLAUSE IS ONE SUBTRACTION OUT** — *"The Rack shares (a)-(c) with the threshold at three"*, but the Rack's X is three MINUS the hand, so emptying a hand under one takes the maximum instead of nothing; the reading is the SLOPE, which gets the Vise, the Rack and Storm World right and is one-directional (it may refuse a draw, never demand one). **P4's `PACE_HORIZON` CLAUSE IS REFUSED and the reason is this repository's own ruling of the same day** (the `TOLL_BEATS` census): a stream times a horizon, and there is no horizon here — every reading prices ONE BEAT. **P4's LAST CLAUSE IS THE DEFENDER DISCOUNT UNDER ANOTHER NAME** and that constant was measured and refused the same day, so it was left. **The Deck (Feb 1996) vs Astral Visionary's four Black Vises 45.9% → 52.7%, +6.8 ±4.4, clear of zero, 92 games flipped to a win against 24 away**; control Big Green vs White Knights 533-467 byte-identical in every arm. **ALL FOUR DECKS P4 NAMES FOR ITS OWN MEASUREMENT ARE UNPLAYABLE** and The Rack's two decks are as well, so the slope is pinned by `tests/ai/` alone |
| ~~`runs_loops`~~ **DONE 2026-09-10** | casting P5 | Wizard | ~~Time Walk priced as a turn; Regrowth on the Walk; the Twister that restarts it~~ REPRODUCED at one seam, three times: a **Wheel of Fortune priced at 4.00 with our hand at seven and theirs at nothing AND at 4.00 with ours at one and theirs at seven** — the same number for a gift of six cards and a gain of six; **Time Walk at 3.00 behind three Serra Angels** (the flat 3.0 `docs/arzakon.strategy` §5 names), 20.70 now; and a **Regrowth taking the Serra Angel over the Time Walk**, 10.00 against 3.00, which is why §3C's loop could never start. `EffectIntent.wheels` — built the same morning and the field P5 asked for — is what the wheel arm reads. The ORDER is a value and not a fourth rule: a turn taken with a returner in hand is credited the card it does not spend, which puts the Walk ahead of the Regrowth beside it. **A WASH ON EVERY WIN RATE and it says so** — +2.2 ±4.1, +3.1 ±3.6, +2.7 ±3.9, +1.6 ±3.8, not one clear of its interval — but all four arms positive, 834 of 4 000 games ending differently and **142 flipped to a win against 46 away**, where a fair toss over 188 sits at 94 ± 6.9; it ships on `holds_x_burn`'s and `reads_pumps`' own precedent. **BOTH DECKS P5 NAMES CANNOT BE PLAYED** (Zuran Orb), and of the twenty-nine decks holding all three pieces only nine load, each with exactly one of each because all three are restricted — so the loop is rare by construction. **P5's OWN MEASURE IS NOT IN THE LAB**: `games.csv` carries no card, so "the median turn the loop first runs" cannot be read from it |
