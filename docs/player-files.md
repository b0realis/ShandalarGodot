...
| **A skin zip you chose** | `user://skin/original_skin.zip` | The 1997 material as ONE ZIP — a `skin/` folder inside with everything the catalogue (`docs/skin-catalogue.txt`) lists — mounted in place at boot by `SkinPack` (`game/skin_pack.gd`) and read as `res://skin/...`; nothing is unpacked. It arrives through **Options → Skin → Choose...** (a file box), by being dropped on the game's window (on the title screen the art shows at once; anywhere else the game offers a restart), or, in a browser, fetched once from `skin/original_skin.zip` beside the page. It wins over the `skin/original_skin.zip` the package ships beside the executable; **Forget my zips** on the same rows deletes it. A zip with an entry outside `skin/` is refused whole. |
| **A card art zip you chose** | `user://skin/cardart.zip` | The second zip — `skin/cardart/<snake_name>.jpg\|png` and nothing else — kept apart from the skin because it is twice the size and from another source (Scryfall). Same three doors (Options → Skin, a drop, the web fetch of `skin/cardart.zip`); which slot a zip lands in is decided by what it HOLDS, never by its name, so a skin zip that carries `cardart/` stays a skin and its pictures are read all the same. |
| **Card art** | `user://original_skin/cardart/<snake_name>.jpg` | Loose Scryfall art crops fetched by `tools/fetch_card_art.py`, or your own: `shivan_dragon.jpg`. Missing art is a graceful placeholder, never an error. |
