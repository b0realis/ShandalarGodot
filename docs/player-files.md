...
| **A skin zip you dropped** | `user://skin/original_skin.zip` | The whole skin as ONE ZIP — a `skin/` folder inside with everything the catalogue (`docs/skin-catalogue.txt`) lists — mounted in place at boot by `SkinPack` (`game/skin_pack.gd`) and read as `res://skin/...`; nothing is unpacked. It arrives by being dropped on the game's window (on the title screen the art shows at once; anywhere else the game offers a restart), or, in a browser, fetched once from `skin/original_skin.zip` beside the page. It wins over the `skin/original_skin.zip` the package ships beside the executable; a zip with an entry outside `skin/` is refused whole. |
...
files beside the executable) → the mounted zip (`res://skin/`, the
dropped one mounted before the shipped one) → `res://assets/original/`.
