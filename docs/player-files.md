...
| **Your skin zips** | `user://skins/*.zip` | The 1997 material as ONE ZIP — a `skin/` folder inside with everything the catalogue (`docs/skin-catalogue.txt`) lists — mounted in place at boot by `SkinPack` (`game/skin_pack.gd`) and read as `res://skin/...`; nothing is unpacked. A zip arrives through **Options → Skin → Choose...** (a file box) or by being dropped on the game's window (on the title screen the art shows at once; anywhere else the game offers a restart), is kept here **under its own name**, and the `skin_zip` key is written to name it as the one worn — the **Skin** row shows that path. With no key the game wears `original_skin.zip` here (what the browser build fetches once from beside the page), else the `skin/original_skin.zip` the package ships beside the executable. To go back to what shipped, delete the zip here (the row shows the path with your home folder as `~`); the browser build, with no folder to open, has a **Forget my zips** button that deletes every zip here and clears the key. A zip with an entry outside `skin/` is refused whole. (The first two-zip build kept its zip in `user://skin/`; it is moved here at the next start.) |
| **The card folder** | `user://cardpacks/*.zip` | Every zip here is a **card pack** — `skin/cardart/<snake_name>.jpg\|png` and nothing else, kept apart from the skin because it is twice the size and from another source (Scryfall) — and **all of them are mounted** at boot, in name order, the first to hold a picture winning: packs for card sets of the future sit beside the first. A pack arrives by the same doors (Options → Skin, a drop, the web fetch of `skin/cardart.zip`) and is kept under its own name; which folder a zip lands in is decided by what it HOLDS, never by its name, so a skin zip that carries `cardart/` stays a skin and its pictures are read all the same. The **Card folder** row shows the folder by its path and names every pack in it; the folder holds a `README.txt` saying this, written the first time you open Options; the `cardpacks_folder` key moves it (below). A pack you no longer want is deleted from the folder; the browser's **Forget my zips** deletes the packs here — only here: a card folder you pointed outside `user://` is yours to empty. |
