...
| Unpreventable damage | "Damage ... can't be prevented or dealt instead to another permanent or player" — bypasses Fog, source-side prevention, protection, Hydra's counters and every redirection gate. | 615.12 | `CardInstance.damage_unpreventable_this_turn` | Whippoorwill (`drk/whippoorwill.gd`) |
...
| Seat-level damage REPLACEMENTS | The whole "the next time a source of your choice would deal damage to you this turn, instead ..." family, as one list of candidates for the CR 616.1 choice below. A handler returns -1 ("carry on with what is left") or >= 0 ("the event ends here, that much was dealt"); an entry is one-shot unless it says `all_turn`. | 614, 616 | `MtgPlayer.damage_replacements`, applied in `MtgGame._land_damage_impl` | Forcefield (`2ed/forcefield.gd`), Dark Sphere (`drk/dark_sphere.gd`), Eye for an Eye (`4ed/eye_for_an_eye.gd`), Nova Pentacle (`leg/nova_pentacle.gd`), Shimian Night Stalker (`leg/shimian_night_stalker.gd`) |
| The DAMAGED SEAT orders them | Every replacement and prevention that would apply to one packet aimed at a player is collected in the order the engine used to run them, the damaged seat picks which applies FIRST, and the rule is put again to what still applies. Taking a Circle of Protection ahead of a Nova Pentacle prevents the damage outright and leaves the Pentacle waiting; taking the Pentacle deflects it and leaves the Circle up. One candidate is not a choice and nobody is asked; none skips the walk. | 616.1 | `MtgGame._has_damage_gates` / `_damage_gates` / `_damage_gate_applies` / `_apply_damage_gate` | Circle of Protection: Red (`2ed/circle_of_protection_red.gd`) against Nova Pentacle (`leg/nova_pentacle.gd`) |
| The PERMANENT'S CONTROLLER orders them | The same CR 616.1 question for a packet aimed at a CREATURE, and the rule's own words put it to the affected OBJECT'S CONTROLLER — not to the seat the damage may end up on, so a Jade Monolith the opponent activated on your blocker is still your blocker's event to order. Eleven gates in the order the fixed chain ran them (Reverberation, Blood of the Martyr, Rock Hydra's counters, protection, Uncle Istvan, Jade Monolith, Personal Incarnation, a whole-damage prevention, Gaseous Form, the source-filtered immunities, a prevention pool); three of them are METERED and order like any other, because a gate is applied ONCE, in full, and the rule is then put again to what is left. 2 damage into a Rock Hydra with three +1/+1 counters and two of its own {R} shields leaves either a 1/1 with its shields or a 3/3 without them. One candidate is not a choice and nobody is asked; none skips the walk, which is every creature in every ordinary combat. | 616.1 | `MtgGame._has_creature_damage_gates` / `_creature_damage_gates` / `_creature_damage_gate_applies` / `_creature_damage_gate_label` / `_apply_creature_damage_gate` / `_mark_creature_damage` | Rock Hydra (`2ed/rock_hydra.gd`) against its own `{R}`; Gaseous Form (`4ed/gaseous_form.gd`) against Jade Monolith (`2ed/jade_monolith.gd`) |
...
| Counter-eating armour | "For each 1 damage that would be dealt to this creature, if it has a +1/+1 counter, remove one and prevent that 1 damage" — point for point, and one of the three METERED gates the creature's controller orders (below). | 614.1 | `CardInstance.damage_eats_counters` | Rock Hydra (`2ed/rock_hydra.gd`) |
...
| A source whose damage all goes elsewhere | "All damage that would be dealt this turn by target sorcery spell is dealt to that spell's controller instead" — a replacement on the SOURCE, and a candidate in the victim's own CR 616.1 ordering on both branches. | 614.1 | `CardInstance.damage_all_redirect_to` | Reverberation (`leg/reverberation.gd`) |
...
| 6b | **LAYER 6, WHOLE, IN TIMESTAMP ORDER**: every ability GRANT and every ability LOSS in one sorted pass — a pump's keywords, a bare keyword grant, a landwalk grant, the until-EOT losses, and every STATIC that grants or removes one named ability (marked `changing_abilities()`, carrying its source's `layer_timestamp`). The later effect wins whichever kind it is, so Radjan Spirit then Flight leaves the Angel flying. It sits ahead of every layer-7 pass, which is also what lets a rules-modifying static that READS an ability (Moat) see the settled answer. | 613.6, 613.7, 613.7b | `_layer_six`, `StaticAbility.changing_abilities`, `CardInstance.layer_timestamp` |
| 7 | COUNTERS: any counter whose NAME parses as `+A/+B` moves P/T by that delta times its count. | layer 7d | `_parse_pt_counter` |
| 8 | All remaining statics (the anthems and everything untagged), in timestamp order. | layer 7c and the rest | `battlefield_with_statics()` |
| 8b | FLOATING STATICS — a `StaticAbility` whose source has left the battlefield — run at the tail of each static sub-pass above, in the same layer order (their layer-6 half runs in 6b with the rest). | the same layers, later timestamp | `_floating_statics`, `_floating_statics_pass` |
| 9 | Floating until-EOT PUMPS (the P/T half; their keywords went in 6b), in creation order. | layer 7c | `_floating` |
| 10 | Floating RAMPAGE grants, BLOCK RESTRICTIONS, granted ACTIVATED abilities, PROTECTION grants and DAMAGE IMMUNITIES — layer 6 too, but in passes of their own, because nothing in the pool REMOVES any of them and their order against a loss is unobservable. | layer 6 | `_rampage_grants`, `_block_restrictions`, `_ability_grants`, `_protection_grants`, `_damage_immunities` |
| 11 | Floating COMBAT-DAMAGE shields (and their all-damage variants). | layer 6 | `_combat_prevention` |
| 12 | P/T SWITCHES, last, in creation order — two switches cancel out. | 613.4e | `_switches` |
...
| Static abilities | A `Callable(game, source)` run every pass; tag it `.changing_types()` or `.setting_base_pt()` to move it into an earlier pass, or `.changing_abilities()` to move it into the timestamped layer-6 pass. The tag is the LAYER, so an ability that does two things in two layers is printed as TWO statics (CR 613.1) — Lord of Atlantis's +1/+1 and its islandwalk. | `StaticAbility` | Crusade (`2ed/crusade.gd`), Nightmare (`2ed/nightmare.gd`), Blood Moon (`drk/blood_moon.gd`), Flight (`2ed/flight.gd`) |
...
timestamp ordering across layers. Layer 6 is fully timestamp-ordered (CR
613.7), statics included, and layer 4 resolves its one dependency in two
waves (CR 613.8); beyond those two there is no dependency analysis, and
there is no "indefinite" duration for base-P/T sets. All of it is contained
in `recalculate` — ROADMAP; the card-visible consequences are in
simplified-cards (*Brine Hag*, *Wall of Tombstones*).
...
| Public hands and private looks | Continuous hand/top-card disclosure is separate from one-time, viewer-authorized looks and reveals. Question preflight carries only information preceding that question; no engine log is a client information source. | Card text | `MtgPlayer.hand_revealed` / `top_card_revealed`, `MtgGame.reveal_information`, `PlayerChoice.information`; `search_library` reveals only when its caller supplies a reveal title | Revelation (`leg/revelation.gd`), Field of Dreams (`leg/field_of_dreams.gd`), Glasses of Urza (`2ed/glasses_of_urza.gd`), Land Tax (`4ed/land_tax.gd`) |
