...
```
./deck_lab.sh --deck-a my_brew.deck --deck-b black_red_raiders.deck --games 10000
...
`./deck_lab.sh --matrix decks/ --games 2000` runs the full round robin —
...
```bash
./deck_lab.sh --matrix decks/ --games 60 --seed 4242 \
...
| `tools/simulate.gd` | the tool: CLI parsing, thread fan-out, reporting |
| `tools/sim_stats.gd` | Wilson intervals, matchup summaries (unit-tested) |
| `tools/svg_charts.gd` | dependency-free SVG charts (bars, histograms, matrix heatmap) |
| `tools/elo_ledger.gd` | the persistent Elo ledger |
