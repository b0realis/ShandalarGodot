extends CardScript
## Black Mana Battery — {4} — Artifact — (4ed, rare)
...
func build() -> CardData:
	return CardData.new("Black Mana Battery", "{4}", Mtg.CardType.ARTIFACT) \
		.activated(ActivatedAbility.new(
			"{2}", true, [ChargeEffect.new()],
			"{2}, {T}: Put a charge counter on Black Mana Battery.")) \
		.mana(ManaAbility.new(Mtg.ManaColor.B).with_any_number_of_counters("charge")) \
		.oracle("{2}, {T}: Put a charge counter on this artifact.\n{T}, Remove any "
			+ "number of charge counters from this artifact: Add {B}, then add an "
			+ "additional {B} for each charge counter removed this way.")
