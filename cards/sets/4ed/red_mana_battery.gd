...
func build() -> CardData:
	return CardData.new("Red Mana Battery", "{4}", Mtg.CardType.ARTIFACT) \
		.activated(ActivatedAbility.new(
			"{2}", true, [ChargeEffect.new()],
			"{2}, {T}: Put a charge counter on Red Mana Battery.")) \
		.mana(ManaAbility.new(Mtg.ManaColor.R).with_any_number_of_counters("charge")) \
		.oracle("{2}, {T}: Put a charge counter on this artifact.\n{T}, Remove any "
			+ "number of charge counters from this artifact: Add {R}, then add an "
			+ "additional {R} for each charge counter removed this way.")
