extends CardScript
## Seafarer's Quay — Land — (leg, uncommon)
## Oracle: Blue legendary creatures you control have "bands with
...
## controller's blue legendary creatures "bands with other
...
func build() -> CardData:
	return CardData.new("Seafarer's Quay", "", Mtg.CardType.LAND) \
		.static_ability(StaticAbility.new(
			_apply,
			"Blue legendary creatures you control have bands with other "
			+ "legendary creatures.")) \
		.oracle("Blue legendary creatures you control have \"bands with other "
			+ "legendary creatures.\"")
...
static func _apply(game: MtgGame, source: CardInstance) -> void:
	for inst in game.all_battlefield():
		...
		if not _is_legendary(inst):
			continue
		if (inst.cur_colors & Mtg.ManaColor.U) == 0:
			continue
