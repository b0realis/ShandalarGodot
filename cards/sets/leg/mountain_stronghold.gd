extends CardScript
## Mountain Stronghold — Land — (leg, uncommon)
## Oracle: Red legendary creatures you control have "bands with
...
## controller's red legendary creatures "bands with other
...
func build() -> CardData:
	return CardData.new("Mountain Stronghold", "", Mtg.CardType.LAND) \
		.static_ability(StaticAbility.new(
			_apply,
			"Red legendary creatures you control have bands with other "
			+ "legendary creatures.")) \
		.oracle("Red legendary creatures you control have \"bands with other "
			+ "legendary creatures.\"")
...
static func _apply(game: MtgGame, source: CardInstance) -> void:
	for inst in game.all_battlefield():
		...
		if not _is_legendary(inst):
			continue
		if (inst.cur_colors & Mtg.ManaColor.R) == 0:
			continue
