class_name FifthEditionPack
extends RefCounted
## Trusted Pack 7 contract. The ZIP supplies data/art, never script paths.
## Fifth Edition is a pure reprint set: it adds printings, never identities.

const ID := "pack-7"
const FILE_NAME := "Pack-7-Fifth-Edition.zip"
const PREFIX := "card_packs/pack_7_fifth_edition/"
const SOURCE := "res://packaging/card_packs/pack_7_fifth_edition/"
const SET_COUNTS := {"5ed": [449, 434]}
const COUNTS := {"published_printings": 449, "named_set_entries": 434,
	"distinct_cards": 434, "pack_card_entries": 449,
	"reprint_entries": 434, "new_rules_identities": 0}
const MAX_ENTRIES := 1196
const SHARED_COUNT := 147
...
## The 147 Ice Age, Homelands and Fallen Empires cards Fifth Edition
## reprints, keyed to the set whose trusted script the pack reuses even
## when that pack is off. Read from shared_names.json in the source folder.
...
## No card is new: every script lives in the original expansion folder.
static func scripts() -> Array:
	var out: Array = []
	var origins := shared()
	for name in origins:
		out.append({"name": name, "set": "5ed",
			"path": "res://cards/sets/%s/%s.gd" % [origins[name], snake(name)]})
	return out
...
static func inspect(path: String) -> Dictionary:
	...
	if not _bounded_zip(path):
		return {"ok": false, "why": "invalid ZIP directory or Pack 7 size limit exceeded"}
...
static func _inspect(reader: ZIPReader) -> Dictionary:
	...
	if not has_art and (got != core or not OS.has_feature("shandalar_test")):
		return {"ok": false, "why": "Rebuild Pack-7-Fifth-Edition.zip with the matching tools/pack_7_fifth_edition.py. Expected %d artwork files and exact metadata, without scripts." % artwork.size()}
	...
	if not (manifest is Dictionary and catalog is Dictionary and cards is Array):
		return {"ok": false, "why": "invalid Pack 7 JSON"}
	...
	for key in trusted:
		if manifest.get(key) != trusted[key]:
			return {"ok": false, "why": "Pack 7 manifest mismatch: " + String(key)}
	...
	if not _same(manifest.get("counts"), COUNTS) or not _same(manifest.get("new_rules_identities"), new_names()):
		return {"ok": false, "why": "Pack 7 counts or names do not match Fifth Edition"}
	var wanted_catalog := {"pack_format": 1, "pack_id": ID, "counts": COUNTS,
		"sets": catalog_sets()}
	if not _same(catalog, wanted_catalog) or cards != printings():
		return {"ok": false, "why": "Pack 7 checklist differs from the trusted Fifth Edition data"}
	var sums := {}
	...
	if not _same(manifest.get("checksums"), expected):
		return {"ok": false, "why": "Pack 7 metadata or artwork checksum does not match"}
