...
| `game/art/card_variant_on.png`, `card_variant_off.png` | round stone artwork-selector medallion with two carved picture frames, 2026-09-24 | ours — original procedural geometry in `tools/draw_our_art.gd` (`_variant_medallion`), no source image | ours (GPL-3.0) |
| `game/art/manalink_globe.png` | the Manalink button's mark, 2026-09-17 | ours — the owner's own picture, reduced to 256 px; the one file in `game/art/` no generator draws | ours (GPL-3.0) |
...
| `game/deck_builder/stone_grind.wav` | the Deck Builder's filter-button cue: 0.250 s, 22 050 Hz, mono, 16-bit PCM, 11 068 B, 2026-09-09 | ours — generated, see below | ours (GPL-3.0) |
...
**What this sound is.** Noise and arithmetic — white noise through a
minimum-phase FIR, shaped by a macro envelope, normalised, written as a
RIFF. It is generated, not recorded, and the generator that makes it is
kept OUTSIDE this repository (`sfx-workshop/`, beside the project) with
the samples and the method, because it is a tool for the next sound and
the next game rather than part of this one. What ships here is the
waveform; `tests/ui/test_deck_sound.gd` asserts its shape.
...
sound of stone scraping on stone.
...
## Portal Pack 6 (2026-09-23)

The checked-in snapshot is Scryfall's `set:por` catalogue and Oracle text,
filtered to English collector numbers 1–215. It contains 215 printings and
200 names; demo/starter variants and foreign-language printings are excluded.
The original-printing Scryfall IDs pin the crop and full-scan downloads to
the same artist and collector number. Downloaded artwork and built ZIPs
remain local. Metadata source: <https://api.scryfall.com/sets/por>.

The large-symbol basic-land presentation preserves the original scan;
Wizards describes Portal's introduction of the symbol in
[Change for the Better](https://magic.wizards.com/en/news/making-magic/change-better-2004-10-04).
The gold set emblem and stone filter medallions follow Portal's circular
gate symbol: a solid center, two rings and ten radial divisions. The visual
reference is [the original set symbol](https://svgs.scryfall.io/sets/por.svg),
cross-checked against a Portal Archangel card scan. The shipped pictures are
our own procedural geometry, not copies or conversions of the SVG;
`game/art/README.md` records their generator and hashes.

Rules use the pinned Oracle text and the shared engine, not an imitation
of Portal's old starter-only timing system. New engine work covers bounded
blocker groups, top-of-library movement, next-turn combat restrictions,
and extra-turn ordering with a turn-specific loss trigger. Semantic AI
estimates use public zones/counts and the player's own hand; library access
and opponent-hand inspection occur only when a card legally instructs a
player to search, look or reveal. No external implementation was copied.
