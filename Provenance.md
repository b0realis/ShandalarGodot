...
| **Forge** — a Magic rules engine and AI in Java, GPL-3.0 | `../forge`, `github.com/Card-Forge/forge.git` (shallow, `b09a3d3f`, read 2026-09-08) | A second reference for engine mechanisms, card decompositions and — above all — AI reads: scripts in `forge-gui/res/cardsfolder/`, effects in `forge-game/.../ability/effects/`, the AI in `forge-ai/`. Read in three engineering notes under `docs/forge/` (combat, casting, rules), every claim pinned to that commit by file and line. Its rules are modern-only (no 1995 legend rule, no prevention step, three old-rules switches to our seven) and its AI hints live inside the card scripts keyed on card names; take the decomposition and the read, never the DSL, the rolls or the names. Nothing ported as of the notes; a port carries a `[forge]` marker at the site naming file, lines and commit. |
...
| `game/icon.png` | the window and taskbar icon | ours (`branding/logo.png` is its master) | ours |
| `game/art/*.png` | the six set glyphs and the damage dagger, 2026-09-09 | ours — generated by `tools/draw_our_art.gd`, which has no input but itself | ours (GPL-3.0) |
| `game/art/fonts/Spectral-Regular.ttf` | **Spectral Regular 2.005**, the body face the game falls back to when no 1997 font is imported, 2026-09-09 | fetched over HTTPS from the family's own upstream directory, Google Fonts' `ofl/spectral` (designed by Production Type) | **SIL Open Font Licence 1.1** — a redistribution licence; `game/art/fonts/OFL.txt` ships beside the file, as it requires |
| `game/deck_builder/stone_grind.wav` | the Deck Builder's filter-button cue: 0.250 s, 22 050 Hz, mono, 16-bit PCM, 11 068 B, 2026-09-09 | ours — synthesised by `tools/make_our_sfx.py`, which reads no file at all; it replaced a supplied download, see below | ours (GPL-3.0) |

This table is the REGISTER — where a file came from and under what
grant. The INVENTORY — every file it carries with its SHA-256, and a
test that holds those hashes to the bytes — is `game/art/README.md`;
`README.md` § Legal names the same files for a reader who opens neither.
The two pictures that predate all of it, `game/icon.png` and
`game/boot_splash.png`, are in this register and not in that inventory.

**Why a font is here at all.** The rules text is set in `font_body`,
which is MPlantin with the 1997 files imported — Monotype's, and never
redistributed by this project. Without an import there was nothing under
it but Godot's default sans. Spectral was chosen from twenty-two free
serifs on 2026-09-09 because its x-height is 0.450 of the em against
MPlantin's 0.450 and its text width is within 1.2%; it is a FLOOR and
never an override, so an imported MPlantin still wins
(`GameSkin.font`, and `game/art/README.md` § The face for the whole
argument). Nothing here is a 1997 file or anyone's restyle of one.

**What this sound is.** Noise and arithmetic. `tools/make_our_sfx.py`
renders it — white noise through a minimum-phase FIR, shaped by a macro
envelope, normalised, written as a RIFF — from a seeded pseudo-random
generator and nothing else. It is standard library only, it is
deterministic, and **it reads no file of any kind**: its self-test
(`tools/test_make_our_sfx.py`) traps `open`, `wave.open` and `os.listdir`
across a whole render and fails if any of them is called, so that claim
is a test rather than a sentence. Run it on a machine with no 1997
install, no download and no network and the same 11 068 bytes come out:

    python3 tools/make_our_sfx.py --only smooth --out game/deck_builder \
                                  --name stone_grind.wav
    ../tools/godot --headless --path . --import

The second line is not optional after the first: what a build plays is
the imported `.sample`, and `stone_grind.wav.import`'s `path=` hash is of
the `res://` path and not of the bytes — so the sidecar needs no edit and
the import does need running, or the pack ships the previous waveform
under the new file's name.

**Which of the five, and who chose it.** Five candidates were synthesised
on ONE axis — how granular the scrape is, from seven hand-placed
resonators (`modal`) through three grain densities (`coarse`, `grain`,
`dense`) to plain filtered noise with no grain train at all (`smooth`) —
with level, length, spectrum, the two accents and the fade held identical
between them, and the owner auditioned all five. They chose `smooth`,
which the measurements ranked FOURTH of the five. `--list` prints what
each one is and `--measure` prints the feature table; `--out <dir>` with
no `--only` renders the whole audition again.

**Why it was made at all: what was here from 2026-09-04 to 2026-09-09.**
The owner's playtest of 2026-09-04 asked for *"a quick stone grinding
sound when pressing the stone filter buttons, based on my sample"* and
supplied `freesound_community077381_scrapingstone83768.mp3` (47 040 B,
2.352 s). That file shipped here, resampled and trimmed, for five days.
Its name identifies where it was downloaded from: **Pixabay sound-effect
83768**, titled *"077381_Scraping Stone"*, uploaded by the Pixabay
account **"Freesound Community"** — a bulk re-upload of freesound.org
material — whose page states *"Free for use under the Pixabay Content
License"*, which permits the use and requires no attribution. (What was
done to it, for the record: GStreamer to 22 050 Hz mono 16-bit
(`mpegaudioparse ! mpg123audiodec ! audioconvert ! audioresample`), the
5 512 frames from sample 1 040, a 4 ms fade-in and a 70 ms raised-cosine
fade-out, normalised x4.60 to a peak of 29 000.)

**Why it could not stay.** The `077381` in that title is *not* a
freesound.org sound id — `freesound.org/s/77381` is an unrelated drum
loop by `bigjoedrummer` (CC0) — so the original uploader on
freesound.org, and whichever of CC0 / CC BY / CC BY-NC they chose there,
were never established and no amount of looking established them. That
matters because the two grants are not the same promise: freesound.org
publishes under Creative Commons, that account re-publishes under
Pixabay's own Content Licence, and the re-licensing is disputed on
Freesound's own forums. If the original was CC BY, an attribution is owed
to somebody nobody here can name. **An obligation that cannot be
discharged is not one to ship on**, so on 2026-09-09 the doctrine that
settled the art settled this too — make our own, from nothing — and the
...
a silent tail, real signal in the middle — so a regeneration that
changed what ships cannot pass quietly.
