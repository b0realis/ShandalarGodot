#!/usr/bin/env python3
"""Build, fetch, or verify the separate Fifth Edition gameplay pack.
...
Fifth Edition reprints cards the game already implements: the pack adds
its 449 printings and artwork, and carries no new rules identity.
"""
...
SOURCE = ROOT / 'packaging/card_packs/pack_7_fifth_edition'
PREFIX = 'card_packs/pack_7_fifth_edition/'
FILE_NAME = 'Pack-7-Fifth-Edition.zip'
DEFAULT_OUT = ROOT.parent / 'shandalar-packs' / FILE_NAME
DEFAULT_ART = ROOT.parent / 'shandalar-packs/cache/pack_7_art'
TOOL = 'pack_7_fifth_edition.py'
WORDMARK = ('┌─┐┌─┐┌─┐┬┌─  ┌─┐', '├─┘├─┤│  ├┴┐   ┌┘', '┴  ┴ ┴└─┘┴ ┴   ┴ ')
CAPTION = ('Shandalar · gameplay pack 7', 'Fifth Edition — local construction')
SETS = {'5ed': (449, 434)}
# Names Fifth Edition shares with Packs 2-4 rather than the 1997 pool; the
# pack ships fallback pictures for these so they render with no other pack.
SHARED_NAMES_FILE = 'shared_names.json'
MAX_ENTRIES = 1196
HINT = (
    'python3 tools/pack_7_fifth_edition.py fetch      # refresh the set data',
    'python3 tools/pack_7_fifth_edition.py fetch-art  # fetch the set artwork',
    'python3 tools/pack_7_fifth_edition.py            # build the local ZIP',
    'python3 tools/pack_7_fifth_edition.py verify     # validate the ZIP',
)
...
def shared_names():
    return set(read_json(SOURCE / SHARED_NAMES_FILE))
...
def fetch_set(code):
    ...
    (SOURCE / 'set.json').write_bytes(json_bytes({
        ...
        'selection': 'Original English numbered printings 1–%d; excludes the five † misprint variants and foreign printings.' % SETS[code][0],
        'retrieved_at': date.today().isoformat(),
    }))
    print('%s: %d printings, %d unique names' %
          (meta['name'], len(records), len({row['name'] for row in records})))

def select_original(rows, code='5ed'):
    """The original English numbered set; the † misprint variants are not printings of their own."""
...
def assembled():
    ...
    for row in rows:
        if row.get('set') not in SETS:
            raise ValueError('Pack 7 must contain only Fifth Edition')
        by_name.setdefault(row['name'], row)
    cards = rows
    if len(rows) != 449 or len(by_name) != 434:
        raise ValueError('Fifth Edition checklist changed; review before building')
    # Every name is a reprint of the 1997 pool or of Packs 2-4. The trusted
    # snapshot avoids needing a running Godot registry or other installed
    # packs; the source metadata and sibling Python helpers remain required
    # when distributing this construction tool.
    reprints = set(read_json(SOURCE / 'reprint_names.json'))
    if len(reprints) != 434 or reprints != set(by_name):
        raise ValueError('Fifth Edition reprint checklist changed; review before building')
    shared = shared_names()
    if len(shared) != 147 or not shared <= reprints:
        raise ValueError('Fifth Edition shared checklist changed; review before building')
    ...
    catalog = {'pack_format': 1, 'pack_id': 'pack-7', 'counts': counts,
               'sets': sets}
    ...
    for index, row in enumerate(cards, 1):
        ...
        if missing:
            ...
            try:
                ...
                # lookup may select a foreign or misprint variant of a card.
        ...
        if index % 10 == 0 or index == len(cards):
            print('Fifth Edition artwork: %d/%d printings' % (index, len(cards)), flush=True)
...
def archive_art_targets(art_dir, cards, fallback):
    """Every printing is namespaced; only the first printing of a shared name supplies fallback art."""
    seen = set()
    for path, row, variant in art_targets(art_dir, cards):
        yield PREFIX + 'art/' + row['set'] + '/' + path.name, path
        key = (row['name'], variant)
        if key not in seen and row['name'] in fallback:
...
def build(out, art_dir=DEFAULT_ART, include_art=True):
    ...
    artwork = []
    if include_art:
        for name, path in archive_art_targets(art_dir, cards, shared_names()):
            if not path.is_file():
    ...
    with tempfile.TemporaryDirectory(prefix='.pack-7-', dir=out.parent) as tmp:
    ...
    print('Pack 7: 434 unique cards · 449 printings -> %s' % out)
...
def verify(path, require_art=True):
    ...
    with zipfile.ZipFile(path) as zf:
        ...
        if (len(entries) > MAX_ENTRIES or any(info.file_size > 8 * 1024 * 1024 for info in entries)
            ...
            raise ValueError('ZIP exceeds Pack 7 size limits')
        names = zf.namelist()
        if len(names) != len(set(names)):
            raise ValueError('duplicate ZIP entries')
        art_names = {name for name, _ in archive_art_targets(Path('unused'), cards, shared_names())}
